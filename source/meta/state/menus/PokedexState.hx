...
class PokedexState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		for (i in 0...dexArray.length)
		{
			...
			if (FileSystem.exists(Paths.getPath('images/pokedex/' + daPoke + '/info.json', TEXT)))
			{
				...
				if (!pokeDexedCharacters.contains(Std.string(swagShit.dex)) && swagShit.dex <= 24)
					name.text = '???';
				maxSelect++;
			}
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (inSubMenu && altBg.y > 7)
			altBg.y -= elapsed / (0.05 / 60);
		if (!inSubMenu && altBg.y < 579)
			altBg.y += elapsed / (0.05 / 60);
	...
	function moveDexSel(diff:Int = 0)
	{
		...
		if (pokeDexedCharacters.contains(Std.string(dexArray[curSelect].dex)) || curSelect >= 24)
		{
			...
			altHtWt.text = 'HT '
				+ (dexArray[curSelect].height == null ? 'NULL' : dexArray[curSelect].height)
				+ '    WT '
				+ (dexArray[curSelect].weight == null ? 'NULL' : dexArray[curSelect].weight);
		}
		else
		{
			nameText.text = '???';
		...
		pokemonNames.forEach(function(spr:FlxSprite)
		{
			...
			if (curSelect > 3)
			{
				...
				if (spr.ID >= -3 + curSelect && spr.ID <= 3 + curSelect)
					spr.visible = true;
				...
				if (distFromEnd <= 4)
				{
					...
					spr.visible = false;
					if (spr.ID >= pokemonNames.length - 7 && spr.ID <= pokemonNames.length)
						spr.visible = true;
				}
			}
			else
